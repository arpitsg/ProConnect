...
from .signals import (
    connection_request_created, connection_request_rejected,
    connection_request_canceled,
    connection_request_viewed, connection_request_accepted,
    connection_removed
)
...
class Connection_Request(models.Model):
    ...
    def __str__(self):
        return "User #%s Connection requested #%s" % (self.from_user_id, self.to_user_id)

    def accept(self):
        """ Accept this friendship request """
        relation1 = Connection.objects.create(
            from_user=self.from_user,
            to_user=self.to_user
        )

        relation2 = Connection.objects.create(
        ...
        Connection_Request.objects.filter(
            from_user=self.to_user,
            to_user=self.from_user
        ).delete()
...
class ConnectionManager(models.Manager):
    ...
    def connections(self, user):
        """ Return a list of all connec """
        key = cache_key('friends', user.pk)
        connections = cache.get(key)

        if connections is None:
            qs = Connection.objects.select_related('from_user', 'to_user').filter(to_user=user).all()
    ...
    def requests(self, user):
        ...
        if requests is None:
            qs = Connection_Request.objects.select_related('from_user', 'to_user').filter(
                to_user=user).all()
            requests = list(qs)
    ...
    def unread_request_count(self, user):
        ...
        if count is None:
            count = Connection_Request.objects.select_related('from_user', 'to_user').filter(
                to_user=user,
    ...
    def add_connection(self, from_user, to_user, message=None):
        ...
        request, created = Connection_Request.objects.get_or_create(
            from_user=from_user,
            to_user=to_user,
        )
    ...
    def are_connections(self, user1, user2):
        ...
        if connections1 and user2 in connections1:
            ...
            try:
                Connection.objects.get(to_user=user1, from_user=user2)
                return True
            except Connection.DoesNotExist:
                return False
